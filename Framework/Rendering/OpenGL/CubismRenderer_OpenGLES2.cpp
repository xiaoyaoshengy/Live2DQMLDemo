...
void CubismClippingManager_OpenGLES2::SetupClippingContext(CubismModel& model, CubismRenderer_OpenGLES2* renderer, GLint lastFBO, GLint lastViewport[4])
{
    ...
    if (usingClipCount > 0)
    {
        if (!renderer->IsUsingHighPrecisionMask())
        {
            ...
            f->glViewport(0, 0, _clippingMaskBufferSize.X, _clippingMaskBufferSize.Y);
...
void CubismRendererProfile_OpenGLES2::SetGlEnableVertexAttribArray(GLuint index, GLint enabled)
{
    ...
    else f->glDisableVertexAttribArray(index);
}
...
void CubismShader_OpenGLES2::GenerateShaders()
{
    ...
    _shaderSets[0]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[0]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[2]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[2]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[3]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[3]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[5]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[5]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[6]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[6]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[8]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[8]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[9]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[9]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[11]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[11]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[12]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[12]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[14]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[14]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[15]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[15]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[17]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[17]->ShaderProgram, "u_channelFlag");
    ...
    _shaderSets[18]->UnifromChannelFlagLocation = f->glGetUniformLocation(_shaderSets[18]->ShaderProgram, "u_channelFlag");
                                                ...
                                                , csmBool invertedMask)
{
    ...
    if (renderer->GetClippingContextBufferForMask() != NULL) // マスク生成時
    {
        ...
        f->glUniform4f(shaderSet->UnifromChannelFlagLocation, colorChannel->R, colorChannel->G, colorChannel->B, colorChannel->A);
...
void CubismRenderer_OpenGLES2::DoDrawModel()
{
    ...
    for (csmInt32 i = 0; i < drawableCount; ++i)
    {
        ...
        if (clipContext != NULL && IsUsingHighPrecisionMask()) // マスクを書く必要がある
        {
            ...
            {
                // 生成したFrameBufferと同じサイズでビューポートを設定
                f->glViewport(0, 0, _clippingManager->GetClippingMaskBufferSize().X, _clippingManager->GetClippingMaskBufferSize().Y);
